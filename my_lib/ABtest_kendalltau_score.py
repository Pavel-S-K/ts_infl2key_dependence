...
def ABtest_kendalltau_score(a: pd.Series, b: pd.Series) -> pd.DataFrame:
    """
    Расчет коэффициента корреляции Kendalltau
    a - cont. target
    b - cont. feature
    """
    ...
    test_results = kendalltau(local_df['feature'].values, local_df['target'].values)

    scores_list.append(test_results[0])
    p_value_list.append(test_results[1])

    scores['kendalltau_score'] = scores_list
